...
public sealed class ToolResultWrapper
{
    ...
    private (string Wrapped, int Size) WrapTextBlock(
        string text,
        string prefix,
        string toolName,
        int remainingBudget)
    {
        var style = _options.Value.ToolResult.Provenance;

        if (style == ProvenanceStyle.Off)
        {
            if (text.Length > remainingBudget)
            {
                const string truncMarker = "…[truncated]";
                int sliceLen = Math.Max(0, remainingBudget - truncMarker.Length);
                var truncated = text[..sliceLen] + truncMarker;
                return (truncated, truncated.Length);
            }
            return (text, text.Length);
        }

        if (style == ProvenanceStyle.Inline)
        {
            string marker = $"[from {prefix}:{toolName}]\n";
            int overhead = marker.Length;
            if (remainingBudget <= overhead)
            {
                var minimal = marker + "[truncated]";
                return (minimal, minimal.Length);
            }
            int textBudget = remainingBudget - overhead;
            if (text.Length <= textBudget)
            {
                var full = marker + text;
                return (full, full.Length);
            }
            else
            {
                const string truncMarker = "…[truncated]";
                int sliceLen = Math.Max(0, textBudget - truncMarker.Length);
                var trunc = marker + text[..sliceLen] + truncMarker;
                return (trunc, trunc.Length);
            }
        }

        // Full style (default — existing behavior)
        {
            ...
            else
            {
                ...
                int sliceLength = textBudget - truncMarker.Length;
                if (sliceLength < 0) sliceLength = 0;
                var truncated = text[..sliceLength] + truncMarker;
                var full = $"{openTag}\n{truncated}\n{closeTag}";
                return (full, full.Length);
            }
        }
